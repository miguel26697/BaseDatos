 ...
 * @author migue
 */
public class RegistroZona implements ActionListener {

    private VistaApar VistaBase;
    private VistaRegistroZona vistarp;
    private conexion con;

    public RegistroZona(VistaApar VistaBase,VistaRegistroZona vistarp, conexion con) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (vistarp.jTextField1.getText().length() != 0 && vistarp.jTextField2.getText().length() != 0) {
            ...
            String actividad = vistarp.jTextField2.getText();
        
            PreparedStatement ps;
                try {
                    ps = con.conexion().prepareStatement("insert into zonas(id_zona,actividad)values('" + Integer.toString(id) + "','" + actividad + "')");
                    ps.executeUpdate();
